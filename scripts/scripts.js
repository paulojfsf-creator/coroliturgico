...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function activateTab(tabId) {
    ...
    tabContents.forEach(content => {
      ...
      const contentTab = content.dataset ? content.dataset.tab : null;
      const match = (contentId === tabId) || (contentTab === tabId);
      content.classList.toggle('active', match);
    });
    try {
      localStorage.setItem('coroActiveTab', tabId);
    } catch (e) {}
  }
  ...
  if (tabButtons.length > 0) {
    ...
    const lastTab = (function(){
      try {
        return localStorage.getItem('coroActiveTab');
      } catch (e) {
        return null;
      }
    })();

    const initialTab = lastTab || tabButtons[0].getAttribute('data-tab');
    if (initialTab) {
